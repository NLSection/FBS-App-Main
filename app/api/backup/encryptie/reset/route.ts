  ...
  try {
    ...
    if (verwijderExternConfig && row?.backup_extern_pad) {
      try {
        ...
        // Verwijder stale verwijzing in backup-meta.json als die naar een .enc.gz bestand wijst
        ...
        if (fs.existsSync(metaPad)) {
          ...
          if (typeof meta.latestBackup === 'string' && meta.latestBackup.endsWith('.enc.gz')) {
