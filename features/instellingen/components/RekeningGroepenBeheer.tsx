...
export default function RekeningGroepenBeheer() {
  ...
  async function laad() {
    ...
    if (r1.ok) setGroepen(await r1.json());
  ...
  async function handleVerwijder() {
    if (bewerkId === null) return;
    if (!confirm('Weet je zeker dat je deze rekeninggroep wilt verwijderen?')) return;
    ...
    return (
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {rekeningen.map(r => {
          ...
          return (
            <button key={r.id} type="button" onClick={() => { const n = new Set(geselecteerd); if (actief) n.delete(r.id); else n.add(r.id); onChange(n); }}
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  const rekNaamById = new Map(rekeningen.map(r => [r.id, r.naam]));

  return (
    <section id="rekening-groepen">
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        <p className="section-title" style={{ margin: 0 }}>Rekeninggroepen</p>
        <InfoTooltip volledigeBreedte tekst="Maak groepen van rekeningen aan om ze als aparte tabbladen op het Dashboard en de Transacties pagina te tonen. Zonder groepen worden alle transacties van alle rekeningen samen weergegeven op het Dashboard. Elke groep toont een eigen Balans Budgetten en Potjes tabel en Samenvatting per Categorie tabel. De volgorde van de groepen bepaalt de tabvolgorde — sleep rijen om de volgorde aan te passen. Het eerste tabblad is de startweergave van het Dashboard." />
      </div>

      {/* Toevoegformulier */}
      {!toonToevoegen ? (
        <button onClick={() => setToonToevoegen(true)} style={{ background: 'none', border: '1px solid var(--border)', color: 'var(--text)', fontSize: 13, padding: '6px 14px', borderRadius: 6, cursor: 'pointer', marginBottom: 12 }}>+ Groep toevoegen</button>
      ) : (
        <form onSubmit={handleToevoegen} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: 16, marginBottom: 16 }}>
          <p style={{ fontSize: 13, fontWeight: 600, marginBottom: 12, color: 'var(--text-h)' }}>Groep toevoegen</p>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="submit" disabled={toevoegBezig || !formNaam.trim()} style={{ ...btnOpslaan, opacity: toevoegBezig || !formNaam.trim() ? 0.5 : 1 }}>Toevoegen</button>
            <button type="button" onClick={() => { setToonToevoegen(false); setFormNaam(''); setFormRekIds(new Set()); setToevoegFout(null); }} style={{ background: 'none', border: '1px solid var(--border)', color: 'var(--text)', fontSize: 13, padding: '7px 16px', borderRadius: 6, cursor: 'pointer' }}>Annuleer</button>
          </div>
      ...
      {groepen.length === 0 ? (
        <p style={{ color: 'var(--text-dim)', fontSize: 13 }}>Geen rekeninggroepen aangemaakt. Maak een groep aan om tabbladen op het Dashboard te tonen.</p>
      ) : (
            ...
            <tbody>
              {groepen.map((groep, index) => (
                <Fragment key={groep.id}>
                  ...
                  {bewerkId === groep.id && (
                    <tr>
                      <td colSpan={3} style={{ padding: '16px 20px', background: 'var(--bg-card)', borderTop: '1px solid var(--border)' }}>
                        <div>
                          <div style={{ marginBottom: 12 }}>
                            <label className={labelCls}>Naam</label>
                          ...
                          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                            <button type="button" onClick={handleOpslaan} disabled={bewerkBezig || !bewerkNaam.trim()} style={{ ...btnOpslaan, opacity: bewerkBezig || !bewerkNaam.trim() ? 0.5 : 1 }}>Opslaan</button>
                            <button type="button" onClick={() => setBewerkId(null)} style={{ background: 'none', border: '1px solid var(--border)', color: 'var(--text)', fontSize: 13, padding: '7px 16px', borderRadius: 6, cursor: 'pointer' }}>Annuleer</button>
                            <div style={{ flex: 1 }} />
                            <button type="button" onClick={handleVerwijder} title="Verwijder" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--red)', padding: 4, display: 'flex', alignItems: 'center' }}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button>
                          </div>
