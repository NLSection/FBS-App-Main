...
export default function BackupCheck() {
  ...
  async function downloadLokaleBackup() {
    const res = await fetch('/api/backup?tabellen=transacties,transactie_aanpassingen,imports,categorieen,budgetten_potjes,budgetten_potjes_rekeningen,subcategorieen,rekeningen,genegeerde_rekeningen,rekening_groepen,rekening_groep_rekeningen,vaste_posten_config,instellingen');
    if (!res.ok) return;
