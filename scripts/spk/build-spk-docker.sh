...
ssh "${NAS_USER}@${NAS_IP}" "sudo mkdir -p /volume1/downloads/FBS-build/spkdir/scripts"
...
ssh "${NAS_USER}@${NAS_IP}" "sudo cp /tmp/INFO /tmp/installer /tmp/start-stop-status /volume1/downloads/FBS-build/"
...
ssh "${NAS_USER}@${NAS_IP}" "sudo cp /tmp/package.tgz /volume1/downloads/FBS-build/"
rm -rf "${TMP_DIR}"

# 4. Draai spksrc toolkit container op NAS om SPK te bouwen
echo "--- SPK bouwen via Docker op NAS ---"
ssh "${NAS_USER}@${NAS_IP}" 'sudo docker run --rm \
