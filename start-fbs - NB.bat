...
@echo off
cd /d "C:\FBS-App\FBS-App-Dev"
echo Git pull uitvoeren...
git pull
echo Git pull voltooid.
if not exist "C:\FBS-App\FBS-App-Dev\node_modules" (
    echo Node modules niet gevonden, npm install uitvoeren...
    npm install
    echo npm install voltooid.
)
timeout /t 2 /nobreak >nul
start wt.exe new-tab --title "FBS Dev Server" -d "C:\FBS-App\FBS-App-Dev" cmd /k "npm run dev" ; new-tab --title "Claude Chrome" -d "C:\FBS-App\FBS-App-Dev" cmd /k "claude --chrome --dangerously-skip-permissions"
